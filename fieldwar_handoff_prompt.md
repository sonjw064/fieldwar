...
## 1-b. 저장소 / 배포 (2026-08-27 추가)

- **GitHub**: https://github.com/sonjw064/fieldwar (main 브랜치, git 저장소는 2026-08-27에 처음 생성함). 커밋 작성자는 `sonjw064 <sonjw0641@gmail.com>`로 로컬(전역 아님, 이 저장소에만) 설정됨
- **배포**: https://fieldwar.onrender.com — Render 무료 플랜, `render.yaml`(Blueprint) 기반으로 GitHub 저장소와 연결되어 있어서 **main 브랜치에 push하면 자동으로 재배포**됨
  - 무료 플랜 특성: 15분간 접속이 없으면 서버가 잠들고, 다음 접속 때 수십 초 정도 깨어나는 지연(cold start)이 있음. 컴퓨터를 꺼도 이 링크는 계속 살아있음(Render 쪽 서버에서 도는 것이므로 로컬 PC 상태와 무관)
  - `server/index.js`가 이미 `process.env.PORT || 3000`을 쓰고 있어서 Render 쪽 포트 자동 할당과 호환됨 (별도 수정 불필요했음)
  - `package.json`에 `engines.node: ">=18.0.0"` 명시해둠
- **로컬 실행**: `cd fieldwar` → `npm start` (또는 `node server/index.js`) → `http://localhost:3000`. `node_modules`는 이미 설치되어 있어서 `npm install` 재실행 불필요(단, 새 컴퓨터/클론 직후라면 먼저 `npm install` 필요)
- `.gitignore`에 `node_modules/`만 등록되어 있음. `Rough/` 폴더(개발자가 준 참고 이미지: 손그림 스케치, 실물 카드 사진)는 저장소에 그대로 포함됨
...
│   ├── data/
│   │   └── cards.json         # 카드 마스터 데이터 (7단계, 2026-08-27 확장: 속성별 공격 5 + 방어 5 + 지형 2 + 효과 2 = 70장). card_design.md가 원본(source of truth), 거기서 수정하면 이 파일에 반영함
│   ├── socket/
...
7. **카드 대량 확장 (7단계, 2026-08-27) — `card_design.md`를 카드 설계의 원본으로 도입**
   - 프로젝트 루트에 `card_design.md` 신설: 이 문서에 카드 이름/코스트/수치/특수효과를 표로 정리해두면, 개발자가 여기서 직접 수정하고 "반영해줘"라고 요청하는 방식으로 카드를 늘려나가기로 함 (앞으로 카드 밸런스/추가는 코드가 아니라 이 md 파일을 먼저 고치는 흐름)
   - 개발자가 채운 1차 내용 기준으로 `server/data/cards.json`을 기존 최소 세트(8장)에서 **70장**으로 교체: 속성별(목/화/토/금/수) 공격 5장(1코스트×2·2코스트×2·3코스트×1) + 방어 5장(동일 분배) + 지형 2장 + 효과 2장
   - **지금 엔진이 처리 가능한 것만 실제로 동작함**: 순수 수치 카드(공격력/방어력만 있는 카드)와 매턴 피해형 효과 카드는 그대로 완전히 동작. 그 외 카드(CC/기절, 화상 등 DoT, 방어력 무시 관통 피해, 광역 피해, 공격력/방어력 버프, 흡혈/고정 회복, 방어 성공 시 반격, 카드별 코스트 증감)는 **일단 이름/코스트/기본 수치 카드로만 동작**하고, 설명(`description`)에 "(효과 미구현)"이라고 표시해둠 — 게임에는 다 뜨지만 특수효과 부분은 아직 아무 일도 안 일어남
   - **지형 보너스 로직 확장**: `Room.getTerrainSynergyBonus()`가 기존에는 "상생 대상 속성"만 강화했는데, 새 지형 카드 설계(자기 속성 + 상생 대상 속성을 동시에 강화)에 맞춰 **자기 자신과 같은 속성도 같이 강화하도록 확장**함. 기존 지형 카드(용암지대 등)에도 동일하게 적용되는, 지형 전체에 걸친 일반 규칙 변경임 (카드별 특수 처리 아님)
   - "코스트 감소/증가형" 지형·효과 카드는 위 확장과 달리 코스트 계산 자체를 건드려야 하는 별개 메커닉이라 이번엔 보류함 (아래 "미구현 메커닉 목록" 참고)
